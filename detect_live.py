...
cv2.createTrackbar('minRadius', 'Dobble Classification', circle_minRadius, circle_xxxRadius,
                   set_minRadius)
cv2.createTrackbar('maxRadius', 'Dobble Classification', circle_maxRadius, circle_xxxRadius,
                   set_maxRadius)
...
while True:
    ...
    if True:
        ...
        if not flag:
            ...
            circles = cv2.HoughCircles(gray2, cv2.HOUGH_GRADIENT, 1.5, 100,
                                       minRadius = circle_minRadius, maxRadius = circle_maxRadius)
            ...
            if circles is not None:
                ...
                for (cx, cy, r) in circles:
                    ...
                    try:
                        ...
                        cv2.putText(output, str(card_id), (x1, y1 - b), text_fontType,
                                    text_fontSize, text_color, text_lineSize, text_lineType)
            ...
            if len(card_list) > 1:
                ...
                for card_pair in itertools.combinations(card_list, 2):
                    ...
                    matching_text = matching_text + (
                            " %02d,%02d=%s" % (card_pair[0], card_pair[1], symbol_label))
                print(matching_text)
                cv2.putText(output, matching_text, (matching_x, matching_y), text_fontType,
                            text_fontSize, text_color, text_lineSize, text_lineType)

                # display real-time FPS counter (if valid)
            if rt_fps_valid == True:
                cv2.putText(output, rt_fps_message, (rt_fps_x, rt_fps_y), text_fontType,
                            text_fontSize, text_color, text_lineSize, text_lineType)
